...
router.get('/:slug/?', async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      'SELECT id, title, slug, body, meta_title, meta_description FROM reports WHERE slug = ? AND is_published = 1 LIMIT 1',
      [req.params.slug]
    );
    ...
    const canonical   = `${SITE_URL}/r/${p.slug}`;
    // Reports allow iframes (their primary purpose) — only strip <script>
...
<head>
  ...
  <style>
    *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .report-header { max-width: 100%; padding: 32px 24px 20px; border-bottom: 2px solid #e5e5e0; }
    h1.report-title { font-family: 'Libre Baskerville', Georgia, serif; font-size: clamp(1.6rem, 3.5vw, 2.4rem); font-weight: 700; line-height: 1.2; color: #0d1b2a; }
    .report-body { padding: 24px 0; }
    .report-body iframe { width: 100%; border: none; }
...
<body>
  ...
  <main>
    <div class="report-header"><h1 class="report-title">${esc(p.title)}</h1></div>
  ...
  <footer class="site-footer">
    ...
    console.error('Reports SSR error:', err);
    res.redirect(302, '/');
  }
});
